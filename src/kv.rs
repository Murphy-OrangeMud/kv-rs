...
use std::fs::File;
use std::io::{Error, ErrorKind, Write, Read, Seek, BufReader, BufWriter, SeekFrom, BufRead};
...
pub struct KvStore {
    kv: HashMap<String, BufReaderWithPos<File>>,
    path: PathBuf,
...
impl KvStore {
    pub fn set(&mut self, key: String, value: String) -> Result<()> {
        ...
        self.kv.insert(key.clone(), BufReaderWithPos { reader: BufReader::new(File::open(self.path.clone())?), pos: self.log_writer.pos - n as u64 });
        Ok(())
    }
    ...
    pub fn open(path: impl Into<PathBuf>) -> Result<KvStore> {
        ...
        let mut kv = HashMap::<String, BufReaderWithPos<File>>::new();
        ...
        while pos < end {
            ...
            match record.cmd {
                ...
                Command::Set => {
                    kv.insert(record.key, BufReaderWithPos::new(File::open(p.clone())?, pos)?);
                }
            }
            pos += x as u64;
        }
        Ok(KvStore { kv: kv, path: p, log_writer: writer })
    }
    ...
    fn compact(&mut self) -> Result<()> {
        ...
        let mut kv = HashMap::<String, BufReaderWithPos<File>>::new();
        for (k, reader) in self.kv.iter_mut() {
            let mut value = String::new();
            reader.read_line(&mut value)?;
            let record: Record = serde_json::from_str(&value)?;
            if record.cmd == Command::Set {
                let n = writer.write(value.as_bytes())?;
                // kv.insert(k.to_owned(), BufReaderWithPos::new(File::open(p.clone())?, writer.pos - n as u64)?);
                writer.flush()?;
            }
        }
